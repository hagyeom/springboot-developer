...
class MemberRepositoryTest extends SpringBootDeveloperApplicationTest {
    ...
    void update() {
        ...
        Member maxRow = memberRepository.getFirstByOrderByIdDesc()
                .orElseThrow(() -> new IllegalStateException("No member found")); // 해당 트랜잭션 안에서 RuntimeException이 발생하면 rollback;

        maxRow.setName("change");
        memberRepository.save(maxRow); // save를 통해 updat, 만약에 하나의 트랜잭션으로 묶으면 save 호출 필요 없음
        // JPA에서 컬렉션에서 꺼내쓰는 것처럼 구현했기 때문에 같은 트랜잭션 내에서는 save를 하지 않아도 자동 저장
